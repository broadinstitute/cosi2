...
cosi2 can be downloaded from https://github.com/broadinstitute/cosi2 .
...
***** Specifying the demographic history
    ...
    Directives that define the demographic history of the populations are specified as follows. They can be supplied in
    any order.  In the entries below, the time =<T>= is measured in generations (which can be fractional) and increases
    going into the past (the present time is generation 0). Labels are used only to provide documentation; they must be enclosed in double
...
***** Special considerations
      ...
      When modeling population splits, if possible, instead of splitting one population into two new ones and zeroing out
      the size of the original one, use a single new population to model lineages that have split off.  Use the original
      population to model remaining lineages.

      When specifying event times, avoid specifying multiple events happening at exactly the same time.  Use small deviations
      to randomize event times into nearby generations.
...
#+BEGIN_EXAMPLE  
  Specifying the model:
    ...
    -R [ --recombfile ] arg         genetic map file (if specified, overrides the
                                    one in paramfile)
    --genmapRandomRegions           for each simulation use a randomly chosen 
                                    subregion of the genetic map
  ...
  Specifying the output format:
    ...
    --tped                   write output to stdout in tped format
                             (alleles: 1=ancestra, 0=derived)
  ...
  Misc options:
    -r N    specify random seed N
    --nTriesPerSim   number of times to try each simulation
...
*** Output formats
    ...
    The .tped format is also supported, using the option =--tped tped-output-prefix=.  In the outputs,
    1 denotes the ancestral allele while 0 denotes the derived allele.
