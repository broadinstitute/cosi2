...
***** Specifying the demographic history
    ...
    : pop_event sweep_mult_standing <pop> <Tend> <selection coefficient> <position of causal allele (0.0-1.0)> <freq at Tend> <selBegPop> <selBegGen>
    ...
    number in the range 0.0-1.0 (not as a chromosome count).  =<selBegPop>= specifies the population in which the
    selection acts.  =<selBegGen>= specifies the generation at which selection begins acting.
