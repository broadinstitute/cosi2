/* $Id: mtwist.c,v 1.1 2011/05/06 23:13:09 sfs Exp $ */
 ...
 *
 * $Log: mtwist.c,v $
 * Revision 1.1  2011/05/06 23:13:09  sfs
 * Switched RNG to mtwist.
 *
...
unsigned long  		mts_seed(mt_state* state);
					/* Choose seed from random input */
void			mts_goodseed(mt_state* state);
					/* Choose seed from more random */
					/* ..input than mts_seed */
static unsigned long   	mts_devseed(mt_state* state, const char* seed_dev);
...
unsigned long	       	mt_seed(void);	/* Choose seed from random input */
 ...
 * current time.
 */
static unsigned long mts_devseed(
    mt_state*		state,		/* State vector to seed */
    const char*		seed_dev)	/* Device to seed from */
    {
    ...
    /*     ranfile = NULL;  */  /* Crude way to disable use of /dev/urandom; saves init time */
    if (ranfile != NULL)
	{
	for (nextbyte = 0;
