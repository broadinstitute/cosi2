...
BOOST_AUTO_TEST_CASE( piecewise_funcs_test ) {
	...
	if(1){
		std::ostringstream inputTest;
		inputTest << "1.0\t20.0\n";
		inputTest << "2.0\t30.0\n";
		std::istringstream inp( inputTest.str() );

		Function<double,double,Piecewise< Const<> > > cf;
		loadFrom( inp, cf );
		PRINT( cf );
		
		Function<double,double,Piecewise< Const<> > > cf2;
		cf2.getPieces().insert( make_pair( 1.0, 20.0 ) );
		cf2.getPieces().insert( make_pair( 2.0, 30.0 ) );
		BOOST_CHECK_EQUAL( cf, cf2 );
	}
