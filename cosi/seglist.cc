...
static level_t seglist_new_level(void) {
  ...
  while ( level < seglist_max_level-1 &&
					...
					seglist_random_gen.random_bit_batched()
					&& seglist_random_gen.random_bit_batched() )
		 level++;
...
const Seg *seglist_last_seg_const( const Seglist *seglist ) {
	const Seg *seg = seglist_first_seg_const( seglist );

	const Seg *nextSeg = NULL;
	while( ( nextSeg = seglist_next_seg_const( seg ) ) != NIL )
		 seg = nextSeg;
	return seg;
}
...
class CallgrindHistogrammer {
public:

	 
	 
	 CallgrindHistogrammer() {
		 CALLGRIND_GET_STAT(EG_IR,0,&startTime);
	 }
	 ~CallgrindHistogrammer() {
		 callgrind_cost_t endTime;
		 CALLGRIND_GET_STAT(EG_IR,0,&endTime);
		 times.push_back( endTime - startTime );
	 }


private:
	 callgrind_cost_t startTime;
	 static std::vector<callgrind_cost_t> times;

	 friend class Summ;
};

std::vector<callgrind_cost_t> CallgrindHistogrammer::times;

static int perc( double i, double tot ) { return (int)( ( i / tot ) * 100 ); }

class Summ {
public:
	 ~Summ() {
		 PRINT( CallgrindHistogrammer::times.size() );
		 using namespace std;
		 vector<callgrind_cost_t>& t = CallgrindHistogrammer::times;
		 
		 sort( t.begin(), t.end() );
		 reverse( t.begin(), t.end() );

		 vector<callgrind_cost_t> partSum;
		 partial_sum( t.begin(), t.end(), back_inserter( partSum ) );

		 double lastPerc = 0.0;
		 for ( unsigned int k = 0; k < partSum.size(); k++ ) {
			 double curPerc = perc(partSum[k],partSum.back());
			 if ( curPerc - lastPerc > 5 ) {
				 PRINT5( k, perc(k,t.size()), t[k], partSum[k], curPerc );
				 lastPerc = curPerc;
			 }
		 }
		 
		 callgrind_cost_t step = partSum.back() / 20;

		 PRINT2( partSum.back(), step );
		 
		 callgrind_cost_t threshold = step;

		 unsigned int i = 0;
		 while ( threshold < partSum.back() ) {
			 while ( partSum[i] < threshold && i < partSum.size() )
					i++;
			 PRINT7( i, perc(i,partSum.size()), threshold, perc(threshold,partSum.back() ), partSum[i], perc(partSum[i],partSum.back()), t[i] );
			 threshold += step;
		 }
	 }
} summ;
#endif  // #ifdef COSI_DEV_CALLGRIND
...
#ifdef COSI_DEV_CALLGRIND	
	CallgrindHistogrammer unionHist;
#endif	
...
bool_t seglist_intersects( const Seglist *seglist1, const Seglist *seglist ) {

  seglist_chk( seglist1 );
  seglist_chk( seglist );
  
  if ( seglist_is_empty( seglist1 ) || seglist_is_empty( seglist ) || !segsumm_intersects( &( seglist1->segsumm ), &( seglist->segsumm ) ) )
		 return False;
	//return True;
	{
		const Seglist *seglists[2] = { seglist1, seglist };
		static Finger *finger1, *finger2;
		if ( !finger1 ) { finger1 = seglist_finger_alloc(); finger2 = seglist_finger_alloc(); }
	 
		Finger *fingers[2] = { finger1, finger2 };
  
		seglist_init_finger( const_cast<Seglist *>(seglist1), finger1, /* init_lens= */ False );
		seglist_init_finger( const_cast<Seglist *>(seglist), finger2, /* init_lens= */ False );

		Seg **first_seg[2] = { &( finger1[ seglist_min_level ].seg ), &( finger2[ seglist_min_level ].seg ) };
		while( *first_seg[0] != NIL && *first_seg[1] != NIL ) {

			/* Local var: which */
			/* The index (0 or 1) of the input seglist that starts earlier (leftmost) on the chromosome. */
			int which = (*first_seg[0])->beg < (*first_seg[1])->beg ? 0 : ( (*first_seg[0])->beg > (*first_seg[1])->beg ? 1 :
																																			( (*first_seg[0])->end > (*first_seg[1])->end ? 0 : 1 ) );
			int other = 1 - which;

			/* skip any segs in seglists[which] that lie completely to the left of seglists[other]. */
			seglist_advance_finger_to_loc( const_cast<Seglist *>(seglists[ which ]), fingers[ which ], (*first_seg[ other ])->beg );

			if ( *first_seg[ which ] != NIL ) seglist_advance_finger_to_next( const_cast<Seglist *>(seglists[ which ]), fingers[ which ] );

			if ( *first_seg[ which ] != NIL ) {
		
				if ( (*first_seg[ which ])->beg > (*first_seg[ other ])->beg )
					 continue;
		
				while( (*first_seg[ other ]) != NIL && (*first_seg[ other ])->end <= (*first_seg[ which ])->end ) {
					if ( (*first_seg[ other ])->beg <= (*first_seg[ other ])->end ) return True;
					*first_seg[ other ] = (*first_seg[ other ])->fwd[ seglist_min_level ].seg; 
				}
		
				if ( (*first_seg[ other ]) != NIL && (*first_seg[ other ])->beg <= (*first_seg[ which ])->end )
					 return True;
		
				*first_seg[ which ] = (*first_seg[ which ])->fwd[ seglist_min_level ].seg; 
			}  /* *first_seg[ which ] != NIL */
		}
  }
...
bool_t seglist_intersect_for_gc( Seglist **seglist, loc_t loc1, loc_t loc2, Seglist **inside, Seglist **outside,
  ...
  *inside = *outside = NULL;
  if ( seglist_is_empty( *seglist ) ) {
		...
		/* [   loc1   loc2    ] */
		/*   a      b      c    */
		Seglist *seglist_ab = NULL, *seglist_a = NULL, *seglist_c = NULL;
