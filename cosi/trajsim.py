...
#
# $File: utils.py $
# $LastChangedDate: 2015-04-03 16:50:47 -0500 (Fri, 03 Apr 2015) $
# $Rev: 4962 $
#
    ...
    def _rpy_plot(self, filename, rpy2, **kwargs):
                ...
                if len(lines) != len(self.traj[gen][0]):
                    if len(lines) != 0:
                        for loc, line in enumerate(lines):
                            ...
                            if start == len(line) - 1:
                                continue
                            r.lines(x=list(range(beginGen - len(line) + start, gen)), y=line[start:],
        ...
        for sp, lines in enumerate(allLines):
            for loc, line in enumerate(lines):
                ...
                plotter.r.lines(x=list(range(beginGen, gen)), y=line[start:],
                        **args.getArgs('lines', None, sp=sp, loc=loc))
        #
