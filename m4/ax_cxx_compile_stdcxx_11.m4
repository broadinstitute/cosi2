...
#   standard; if necessary, add switches to CXX11_CXXFLAGS to enable support.
...
AC_DEFUN([AX_CXX_COMPILE_STDCXX_11], [dnl
  ...
  if test x$ac_success = xno; then
    for switch in -std=gnu++11 -std=gnu++0x; do
      ...
      if eval test x\$$cachevar = xyes; then
        AC_SUBST(CXX11_CXXFLAGS,[$switch])
        ac_success=yes
        break
      fi
    done
  ...
  if test x$ac_success = xno; then
    AC_MSG_NOTICE([C++11 features not supported, disabling corresponding compiler flags.])
  fi
])
