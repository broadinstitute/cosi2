...
if test x"$with_boost" = x; then
    with_boost=$srcdir/boost
	 own_boost=yes
	 ZDRASTEVAM=yes
	 AC_MSG_NOTICE([Boost: Using own copy of boost])
fi
...
[if test x"$own_boost" == xyes; then
  AC_MSG_NOTICE([program_options: only checking for header])
  BOOST_FIND_HEADER([boost/program_options.hpp])
  AC_MSG_NOTICE([program_options: done checking for header])
