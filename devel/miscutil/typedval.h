...
template <class T> inline cosi_double cosi_fabs( const TypedVal<T,cosi_double>& val ) { return ::fabs( ToDouble( val ) ); }
...
struct TypedValRel: public TypedVal<T,V> {
	 ...
	 T& operator*=( const factor_t& factor ) { this->val *= factor.val; return (T&)*this; }
	 const T operator*( const factor_t& f ) const { return T( this->val * f.val ); }
};
...
template <class T, typename V> inline
const T operator*( const factor_t& f, const TypedValRel<T,V>& rv ) { return T( f.val * rv.val  ); }
